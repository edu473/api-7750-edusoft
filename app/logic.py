...
# --- GESTIÓN DE CONCURRENCIA (CORREGIDO) ---
# Mantenemos el Lock para evitar escrituras simultáneas, que es la causa del problema original.
BNG_WRITE_LOCKS = {bng: asyncio.Lock() for bng in DEVICES}

# --- FUNCIONES INTERNAS (WORKERS) ---
# Volvemos a usar `with gNMIclient(...)` que es la forma segura de manejar la conexión.
...
async def bulk_update_subscriber_state_logic(bng: str, request_data: models.BulkUpdateStateRequest):
    ...
    if request_data.state == "disable" and customers_to_process:
        # Pysros tasks are synchronous and should be run in a thread
        clear_tasks = [
